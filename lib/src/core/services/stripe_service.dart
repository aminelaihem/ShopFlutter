...
class StripeService {
  ...
  static Future<void> initialize() async {
    try {
      ...
      debugPrint('Stripe initialisé avec la clé: ${StripeConfig.publishableKey.substring(0, 20)}...');
    } catch (e) {
  ...
  static Future<PaymentIntent> createPaymentIntent({
    ...
    try {
      debugPrint('Création du PaymentIntent pour ${amount/100}€ via API Stripe');
  ...
  static Future<PaymentIntent> confirmPayment({
    ...
    try {
      ...
      if (e.toString().contains('402')) {
        ...
        debugPrint('Vérifiez que votre compte Stripe est activé pour les paiements');
      }
  ...
  static Map<String, dynamic> _simulateCardResponse(String? cardNumber, int amount, String currency) {
  ...
  static Future<void> detachPaymentMethod({
    ...
    try {
      ...
      throw Exception('Erreur lors de la suppression de la méthode de paiement: $e');
    }
  }
}