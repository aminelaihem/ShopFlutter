...
void main() {
  group('FetchProduct Use Case', () {
    ...
    test(
      'devrait retourner un produit quand la récupération réussit',
      () async {
        // Arrange
        const productId = 1;
        ...
        when(
          mockRepository.fetchProduct(productId),
    ...
    test('devrait propager l\'exception du repository', () async {
      ...
      when(
        mockRepository.fetchProduct(productId),
      ).thenThrow(Exception('Produit introuvable'));

      // Act & Assert
      expect(() => fetchProductUseCase(productId), throwsA(isA<Exception>()));
    });
    ...
    test('devrait gérer les IDs négatifs', () async {
      ...
      when(
        mockRepository.fetchProduct(productId),
      ).thenThrow(Exception('ID invalide'));
